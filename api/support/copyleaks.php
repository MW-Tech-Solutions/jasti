...
function jasti_ensure_plagiarism_scan_table(PDO $pdo): void
{
    static $ready = false;
    if ($ready) {
        return;
...
function jasti_plagiarism_integration_settings(PDO $pdo): array
{
    $settings = jasti_settings($pdo);

    $provider = trim((string) ($settings['plagiarism_provider'] ?? jasti_env('PLAGIARISM_PROVIDER', 'copyleaks')));
    $enabledValue = (string) ($settings['plagiarism_enabled'] ?? jasti_env('PLAGIARISM_ENABLED', '0'));
    $sandboxValue = (string) ($settings['plagiarism_sandbox'] ?? jasti_env('COPYLEAKS_SANDBOX', '1'));
    $requireCompletionValue = (string) ($settings['plagiarism_require_completion'] ?? jasti_env('PLAGIARISM_REQUIRE_COMPLETION', '1'));
    $apiEmail = trim((string) ($settings['plagiarism_api_email'] ?? jasti_env('COPYLEAKS_EMAIL', '')));
    $apiKey = trim((string) ($settings['plagiarism_api_key'] ?? jasti_env('COPYLEAKS_API_KEY', '')));
    $webhookSecret = trim((string) ($settings['plagiarism_webhook_secret'] ?? jasti_env('COPYLEAKS_WEBHOOK_SECRET', '')));
...
function jasti_ensure_plagiarism_webhook_secret(PDO $pdo): string
{
    ...
    jasti_upsert_setting($pdo, 'plagiarism_webhook_secret', $secret);

    return $secret;
}

function jasti_copyleaks_webhook_url(PDO $pdo, string $event): string
{
    $secret = jasti_ensure_plagiarism_webhook_secret($pdo);
    return jasti_backend_url('api/integrations/copyleaks_webhook.php?token=' . urlencode($secret) . '&event=' . urlencode($event));
}

function jasti_copyleaks_status_webhook_template(PDO $pdo): string
{
    $secret = jasti_ensure_plagiarism_webhook_secret($pdo);
    return jasti_backend_url('api/integrations/copyleaks_webhook.php?token=' . urlencode($secret) . '&event={STATUS}');
}

function jasti_copyleaks_sdk_bootstrap(): void
{
    $autoloadPath = jasti_root_path('vendor/autoload.php');
...
function jasti_copyleaks_client(): \Copyleaks\Copyleaks
{
    jasti_copyleaks_sdk_bootstrap();
...
function jasti_copyleaks_token(PDO $pdo): \Copyleaks\CopyleaksAuthToken
{
    $settings = jasti_plagiarism_integration_settings($pdo);
    ...
    $client = jasti_copyleaks_client();
    return $client->login((string) $settings['api_email'], (string) $settings['api_key']);
}

function jasti_plagiarism_file_sha256(array $file): string
{
...
function jasti_create_web_plagiarism_scan(PDO $pdo, int $userId, array $file): array
{
    ...
    $externalScanId = 'jasti_' . time() . '_' . bin2hex(random_bytes(6));
    $sha256 = jasti_plagiarism_file_sha256($file);
    ...
    try {
        $token = jasti_copyleaks_token($pdo);
        $webhooks = new \Copyleaks\SubmissionWebhooks(
            jasti_copyleaks_status_webhook_template($pdo),
            jasti_copyleaks_webhook_url($pdo, 'new-results')
        );
    ...
    return jasti_plagiarism_scan_by_token($pdo, $userId, $draftToken) ?? [];
}

function jasti_plagiarism_scan_by_token(PDO $pdo, int $userId, string $draftToken): ?array
{
    jasti_ensure_plagiarism_scan_table($pdo);
...
function jasti_plagiarism_status_message(array $scan): string
{
...
function jasti_validate_plagiarism_scan_for_submission(PDO $pdo, int $userId, string $draftToken, string $sourceSha256): array
{
    jasti_ensure_plagiarism_scan_table($pdo);

    $scan = jasti_plagiarism_scan_by_token($pdo, $userId, $draftToken);
    if ($scan === null) {
...
function jasti_recent_plagiarism_scans(PDO $pdo, int $limit = 20): array
{
    jasti_ensure_plagiarism_scan_table($pdo);
...
function jasti_update_scan_row(PDO $pdo, string $externalScanId, array $updates): void
{
    jasti_ensure_plagiarism_scan_table($pdo);
...
function jasti_handle_copyleaks_completed_webhook(PDO $pdo, array $payload): array
{
    jasti_copyleaks_sdk_bootstrap();
    ...
    jasti_update_scan_row($pdo, $webhook->scanId, [
        'status' => 'completed',
...
function jasti_handle_copyleaks_error_webhook(PDO $pdo, array $payload): array
{
    jasti_copyleaks_sdk_bootstrap();
...
function jasti_handle_copyleaks_status_webhook(PDO $pdo, array $payload, string $status): array
{
    jasti_update_scan_row($pdo, (string) ($payload['scanId'] ?? ''), [
        'status' => $status,
        'raw_response_json' => json_encode($payload, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE),
    ]);
