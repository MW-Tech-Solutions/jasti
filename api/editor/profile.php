...
if (!jasti_has_editor_workspace_role((array) ($user['roles'] ?? []))) {
...
$schemaReady = jasti_table_exists($pdo, 'editors')
...
function jasti_decode_array_field(mixed $value): array
{
    if (is_array($value)) {
        return $value;
    }
...
if (!$schemaReady) {
    jasti_json(['message' => 'Editor application tables are not available yet. Run the editor migration SQL first.'], 503);
}
...
$qualifications = array_values(array_filter(jasti_decode_array_field($data['qualifications'] ?? []), static function ($entry): bool {
...
$expertiseItems = array_values(array_filter(jasti_decode_array_field($data['expertise'] ?? []), static function ($entry): bool {
    return is_array($entry)
        && trim((string) ($entry['research_area'] ?? '')) !== '';
}));

$journalExperiences = array_values(array_filter(jasti_decode_array_field($data['journal_experiences'] ?? []), static function ($entry): bool {
    return is_array($entry)
...
try {
    ...
    jasti_json(['message' => 'Unable to update editor application.', 'error' => $exception->getMessage()], 500);
}

jasti_log($pdo, $userId, 'updated editor application', 'editors', $userId);
jasti_json([
