...
if (!$isAdmin && !$isEIC) {
    jasti_json(['message' => 'Unauthorized. Only admin or editor-in-chief can approve editor applications.'], 403);
}
...
if ((int) ($application['application_completed'] ?? 0) !== 1 || empty($application['cv_file'])) {
    jasti_json(['message' => 'Editor application must be completed and include a PDF CV before approval.'], 400);
}
...
jasti_json(['message' => 'Editor application approved.', 'editor_id' => $editorId], 200);
