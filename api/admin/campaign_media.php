...
if (!isset($_FILES['file']) || !is_array($_FILES['file'])) {
    jasti_json(['message' => 'File is required.'], 422);
}

$file = $_FILES['file'];
if (($file['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
    jasti_json(['message' => jasti_upload_error_message((int) ($file['error'] ?? UPLOAD_ERR_NO_FILE))], 422);
}

$maxBytes = (int) jasti_env('MAX_UPLOAD_SIZE_BYTES', '10485760');
if ((int) ($file['size'] ?? 0) <= 0) {
    jasti_json(['message' => 'Empty upload.'], 422);
}
if ((int) $file['size'] > $maxBytes) {
    jasti_json(['message' => 'File size exceeds limit.'], 422);
}

$tmp = (string) ($file['tmp_name'] ?? '');
if ($tmp === '' || !is_file($tmp)) {
    jasti_json(['message' => 'Upload temp file is missing.'], 422);
}

$bytes = file_get_contents($tmp);
if ($bytes === false) {
    jasti_json(['message' => 'Unable to read uploaded file.'], 500);
}
...
if (!in_array($mediaType, ['image', 'file'], true)) {
    jasti_json(['message' => 'Invalid media_type.'], 422);
}
...
jasti_json(['message' => 'Media saved.', 'media' => $row]);
