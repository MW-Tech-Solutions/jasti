...
if (!in_array($role, $allowedRoles, true)) {
    jasti_json(['message' => 'Role must be author, reviewer, or editor.'], 422);
}

if ($role === 'editor' && $editorType === '') {
    jasti_json(['message' => 'Editor type is required when registering as editor.'], 422);
}
...
if ($role === 'editor' && !in_array($editorType, $validEditorTypes, true)) {
    jasti_json(['message' => 'Invalid editor type. Must be one of: ' . implode(', ', $validEditorTypes)], 422);
}
...
try {
    ...
    // Every account keeps author access; reviewer/editor access is added on the same email account.
    jasti_assign_account_role($pdo, $userId, $role, $roleDescriptions[$role]);
