...
if (!in_array('editor', $roles, true) && !in_array('admin', $roles, true)) {
    ajasti_json(['message' => 'Only editors and administrators can update plagiarism scores.'], 403);
}
...
if ($manuscriptId <= 0) {
    ajasti_json(['message' => 'A manuscript is required.'], 422);
}

$score = null;
if ($rawScore !== '') {
    if (!is_numeric($rawScore)) {
        ajasti_json(['message' => 'Plagiarism score must be numeric.'], 422);
    }
    $score = round((float) $rawScore, 2);
    if ($score < 0 || $score > 100) {
        ajasti_json(['message' => 'Plagiarism score must be between 0 and 100.'], 422);
    }
}
...
ajasti_log($pdo, $userId, 'updated plagiarism score', 'manuscripts', $manuscriptId);
