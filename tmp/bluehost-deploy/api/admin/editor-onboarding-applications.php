...
if (!$isAdmin && !$isEIC) {
    ajasti_json(['message' => 'Unauthorized. Only admin or editor-in-chief can view editor applications.'], 403);
}
...
if ($completedOnly) {
    $where .= ' AND e.application_completed = 1';
}

$stmt = $pdo->prepare(
    'SELECT e.editor_id, e.user_id, e.first_name, e.last_name, e.email, e.country, e.institution, e.department, e.position,
            e.editor_role, e.cv_file, e.publication_list_file, e.application_completed, e.status, e.date_registered,
            e.reviewed_at, e.reviewed_by, e.rejection_reason, e.acceptance_notes,
            ru.first_name AS reviewer_first_name, ru.last_name AS reviewer_last_name
     FROM editors e
     LEFT JOIN users ru ON ru.user_id = e.reviewed_by
     WHERE ' . $where . '
     ORDER BY e.date_registered ASC
...
$countStmt = $pdo->prepare('SELECT COUNT(*) FROM editors e WHERE ' . $where);
