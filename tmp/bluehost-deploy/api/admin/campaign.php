...
if (!campaign_admin_table_exists($pdo, 'campaign_site') || !campaign_admin_table_exists($pdo, 'campaign_media')) {
    ajasti_json(['message' => 'Campaign CMS tables are not available yet. Run the campaign migration SQL first.'], 503);
}
...
if ($action === '') {
    ajasti_json(['message' => 'Action is required.'], 422);
}
...
try {
    if ($action === 'upsert_site') {
        ...
        if (!in_array($collection, $allowed, true)) {
            ajasti_json(['message' => 'Unsupported collection.'], 422);
        }
        ...
        ajasti_json(['message' => 'Unsupported action.'], 422);
    }
