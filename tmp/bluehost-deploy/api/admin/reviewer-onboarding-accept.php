...
if (!$isAdmin && !$isEIC) {
    ajasti_json(['message' => 'Unauthorized. Only admin or editor-in-chief can approve reviewer applications.'], 403);
}
...
if ((int) ($application['application_completed'] ?? 0) !== 1 || empty($application['cv_file'])) {
    ajasti_json(['message' => 'Reviewer application must be completed and include a PDF CV before approval.'], 400);
}
...
ajasti_json(['message' => 'Reviewer application approved.', 'reviewer_id' => $reviewerId], 200);
