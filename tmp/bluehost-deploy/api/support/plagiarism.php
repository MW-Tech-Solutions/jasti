...
function ajasti_ensure_manuscript_text_index_table(PDO $pdo): void
{
    static $ready = false;
    if ($ready) {
        return;
...
function ajasti_run_process(array $command, ?string $workingDirectory = null): array
{
    $descriptors = [
        0 => ['pipe', 'r'],
        1 => ['pipe', 'w'],
...
function ajasti_remove_directory(string $directory): void
{
    if (!is_dir($directory)) {
        return;
    }
...
function ajasti_extract_pdf_text(string $path): string
{
    if (!is_file($path)) {
        return '';
    }

    $result = ajasti_run_process(['pdftotext', '-enc', 'UTF-8', '-nopgbrk', $path, '-']);
...
function ajasti_extract_docx_text(string $path): string
{
    if (!is_file($path)) {
        return '';
    }
...
function ajasti_extract_legacy_word_text(string $path): string
{
    if (!is_file($path)) {
        return '';
    }

    $tempRoot = rtrim(sys_get_temp_dir(), DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'ajasti_doc_' . bin2hex(random_bytes(8));
    ...
    if (!mkdir($profileDir, 0700, true) && !is_dir($profileDir)) {
        ajasti_remove_directory($tempRoot);
        return '';
    }
...
function ajasti_extract_text_from_manuscript_file(string $path, string $mimeType = '', string $originalName = ''): string
{
    ...
    return match (true) {
        $mimeType === 'application/pdf', $extension === 'pdf' => ajasti_extract_pdf_text($path),
        $mimeType === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', $extension === 'docx' => ajasti_extract_docx_text($path),
        $mimeType === 'application/msword', $extension === 'doc' => ajasti_extract_legacy_word_text($path),
...
function ajasti_normalize_manuscript_text(string $text): string
{
...
function ajasti_similarity_stopwords(): array
{
...
function ajasti_manuscript_tokens(string $normalizedText): array
{
    if ($normalizedText === '') {
        return [];
    }

    $stopwords = ajasti_similarity_stopwords();
...
function ajasti_manuscript_shingles(array $tokens, int $size = 5): array
{
...
function ajasti_absolute_storage_path(string $publicPath): ?string
{
    ...
    return ajasti_root_path(ltrim($publicPath, '/'));
}

function ajasti_upsert_manuscript_text_index(PDO $pdo, int $manuscriptId, string $normalizedText): void
{
    ajasti_ensure_manuscript_text_index_table($pdo);
    $stmt = $pdo->prepare(
...
function ajasti_build_archive_similarity_report(PDO $pdo, string $normalizedText, ?int $excludeManuscriptId = null): array
{
    ajasti_ensure_manuscript_text_index_table($pdo);

    $candidateTokens = ajasti_manuscript_tokens($normalizedText);
    $candidateShingles = ajasti_manuscript_shingles($candidateTokens);
    ...
    foreach ($rows as $row) {
        ...
        if ($sourceText === '') {
            $absolutePath = ajasti_absolute_storage_path((string) ($row['manuscript_file_path'] ?? ''));
            if ($absolutePath !== null && is_file($absolutePath)) {
                $sourceRawText = ajasti_extract_text_from_manuscript_file($absolutePath, '', basename($absolutePath));
                $sourceText = ajasti_normalize_manuscript_text($sourceRawText);
            }

            if ($sourceText === '') {
                $sourceText = ajasti_normalize_manuscript_text(
                    trim(
                        (string) ($row['title'] ?? '') . ' ' .
                        (string) ($row['abstract'] ?? '') . ' ' .
                        (string) ($row['keywords'] ?? '')
            ...
            if ($sourceText !== '') {
                ajasti_upsert_manuscript_text_index($pdo, (int) $row['manuscript_id'], $sourceText);
            }
        }
        ...
        $sourceTokens = ajasti_manuscript_tokens($sourceText);
        $sourceShingles = ajasti_manuscript_shingles($sourceTokens);
        if ($sourceShingles === []) {
            continue;
        }
...
function ajasti_calculate_uploaded_manuscript_similarity(PDO $pdo, array $file): array
{
    ...
    $rawText = ajasti_extract_text_from_manuscript_file($tmpPath, $mimeType, (string) ($file['name'] ?? ''));
    $normalizedText = ajasti_normalize_manuscript_text($rawText);
    $tokenCount = count(ajasti_manuscript_tokens($normalizedText));
    if ($tokenCount < 40) {
    ...
    $report = ajasti_build_archive_similarity_report($pdo, $normalizedText);
