...
$schemaReady = ajasti_table_exists($pdo, 'reviewers')
...
function ajasti_decode_reviewer_array(mixed $value): array
{
    if (is_array($value)) {
        return $value;
    }
...
if (!$schemaReady) {
    ajasti_json(['message' => 'Reviewer application tables are not available yet. Run the reviewer migration SQL first.'], 503);
}
...
$specializations = array_values(array_filter(array_map('trim', (array) ajasti_decode_reviewer_array($data['specializations'] ?? []))));

$qualifications = array_values(array_filter(ajasti_decode_reviewer_array($data['qualifications'] ?? []), static function ($entry): bool {
...
$expertise = array_values(array_filter(ajasti_decode_reviewer_array($data['expertise'] ?? []), static function ($entry): bool {
...
$journalExperiences = array_values(array_filter(ajasti_decode_reviewer_array($data['journal_experiences'] ?? []), static function ($entry): bool {
    return is_array($entry) && trim((string) ($entry['journal_name'] ?? '')) !== '';
}));

$conflicts = array_values(array_filter(ajasti_decode_reviewer_array($data['conflicts'] ?? []), static function ($entry): bool {
    return is_array($entry)
...
if (isset($_FILES['orcid_screenshot_file']) && is_array($_FILES['orcid_screenshot_file'])) {
    $orcidScreenshotFile = ajasti_store_uploaded_file(
...
try {
    ...
    ajasti_json(['message' => 'Unable to update reviewer application.', 'error' => $exception->getMessage()], 500);
}

ajasti_log($pdo, $userId, 'updated reviewer application', 'reviewers', $userId);
ajasti_json([
