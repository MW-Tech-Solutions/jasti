...
if (isset($_FILES['signed_form']) && is_array($_FILES['signed_form']) && (int) ($_FILES['signed_form']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE) {
    $uploadedSignedFile = ajasti_store_uploaded_file(
        $_FILES['signed_form'],
        'uploads/copyright',
        'copyright_form',
        [
...
if ($manuscriptId <= 0 || $signedFilePath === '') {
    ajasti_json(['message' => 'Manuscript and signed copyright form are required.'], 422);
}
...
if (!$ownership->fetch()) {
    ajasti_json(['message' => 'A payment record is required before uploading the copyright form.'], 422);
}
...
ajasti_log($pdo, $userId, 'submitted copyright form', 'copyright_forms', $formId);
ajasti_json(['message' => 'Copyright form uploaded successfully.']);
