...
function createCaptchaChallenge(): CaptchaChallenge {
  ...
  const text = Array.from({ length: 6 }, () => alphabet[Math.floor(Math.random() * alphabet.length)]).join("")
  return { text }
}
...
export default function PortalApp({ onAuthenticated, page = "portal" }: PortalAppProps) {
  ...
  const [mode, setMode] = React.useState<PortalMode>(() => {
    if (isDedicatedAuthorPage) return "register"
    if (isDedicatedReviewerPage) return "register"
    if (isDedicatedEditorPage) return "editor_register"
    if (isDedicatedAuthorLoginPage || isDedicatedReviewerLoginPage || isDedicatedEditorLoginPage || isDedicatedAdminLoginPage) return "login"
    return "login"
  })
  ...
  React.useEffect(() => {
    ...
    for (let index = 0; index < 8; index += 1) {
      context.strokeStyle = `rgba(11, 111, 164, ${0.12 + index * 0.04})`
      context.beginPath()
    ...
    captcha.text.split("").forEach((character, index) => {
      ...
      const rotation = Math.random() * 0.45 - 0.225
      context.save()
      context.translate(x, y)
      context.rotate(rotation)
      context.fillStyle = "#083b5c"
  ...
  const authHeading = registrationComplete
    ...
    : isDedicatedEditorPage
      ? {
          title: "Registration",
          description: "Apply to join the JASTI editorial team and upload your CV (PDF).",
        }
        ...
        >
          ...
          {mode === "verification" ? (
            ...
            <div className="mx-auto max-w-xl space-y-6 text-center">
              <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-6 py-8">
                <p className="text-sm font-semibold uppercase tracking-[0.16em] text-emerald-700">Registration successful</p>
                ...
                {pendingVerification.email ? <p className="mt-3 text-sm text-slate-500">Verification email sent to {pendingVerification.email}.</p> : null}
                {verificationExpiryLabel ? <p className="mt-2 text-sm text-slate-500">{verificationExpiryLabel}</p> : null}
              </div>
            ...
            <form className="mx-auto mt-5 max-w-5xl space-y-4 text-left" onSubmit={handleRegister}>
              ...
              <div className="grid gap-3 lg:grid-cols-[200px_minmax(0,1fr)] lg:items-end">
                <div className="space-y-1.5">
                  <Label htmlFor="captcha" className="text-[13px] font-semibold text-slate-800">Captcha</Label>
                  <div className="flex h-12 items-center justify-center rounded-[0.95rem] border border-slate-200 bg-slate-50 px-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.9)]">
                    <canvas ref={captchaCanvasRef} width={190} height={46} className="max-w-full" />
                  </div>
                </div>
                <div className="space-y-1.5">
              ...
              <p className="text-center text-[12px] text-slate-500">
                Already registered?{" "}
                <button type="button" onClick={returnToLogin} className="font-semibold text-[#0b6fa4] hover:underline">
                  Sign in
                </button>
              </p>
            </form>
