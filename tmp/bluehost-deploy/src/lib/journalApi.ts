...
function stripPortFromOrigin(origin: string) {
...
function defaultApiBaseUrl() {
  ...
  if (import.meta.env.DEV) {
    return `${stripPortFromOrigin(origin)}/ajasti/api`
  }

  if (normalizedPath === "/ajasti" || normalizedPath.startsWith("/ajasti/")) {
    return `${origin}/ajasti/api`
  }
...
function isHostedLocalOnlyApiUrl(value: string) {
  ...
  try {
    ...
    return localHosts.has(configuredHost) && !localHosts.has(currentHost)
  } catch {
    return false
  }
}
...
function configuredApiBaseUrl() {
  ...
  if (runtimeConfig) {
    ...
    if (!isHostedLocalOnlyApiUrl(normalizedRuntimeUrl)) {
      return normalizedRuntimeUrl
    }
  }
  const runtimeMeta = document.querySelector('meta[name="ajasti-api-base"]')?.getAttribute("content")?.trim() || ""
  const normalizedMetaUrl = normalizeConfiguredUrl(runtimeMeta)
  if (!isHostedLocalOnlyApiUrl(normalizedMetaUrl)) {
...
export async function getFeaturedArticles() {
  const { data } = await journalApi.get("/public/articles.php")
...
export async function sendMessage(payload: Record<string, unknown>) {
  ...
  return data as { message: string; message_id: number }
}
...
export async function submitManuscript(payload: Record<string, unknown>, options?: UploadOptions) {
  ...
  return data as { message: string; manuscript_id: number }
}
...
export async function initializePaystackPayment(payload: { manuscript_id: number; amount: number }) {
...
export async function submitReview(payload: Record<string, unknown>) {
  const { data } = await journalApi.post("/reviewer/reviews.php", payload)
