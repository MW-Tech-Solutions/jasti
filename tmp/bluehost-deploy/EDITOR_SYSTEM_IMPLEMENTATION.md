...
### Permission Logic
```php
function ajasti_can_access_dashboard($pdo, $userId, $dashboardName)
function ajasti_can_edit_dashboard($pdo, $userId, $dashboardName)
function ajasti_get_dashboard_url($editorTypeName)
```
...
- `ajasti_ensure_editor_types()` - Create tables if missing
- `ajasti_editor_types()` - Get all editor types
- `ajasti_editor_type_by_id()` - Get specific type
- `ajasti_editor_type_by_name()` - Get type by name
- `ajasti_user_editor_profile()` - Get user's editor profile
- `ajasti_create_editor_profile()` - Create new profile
- `ajasti_can_access_dashboard()` - Check read access
- `ajasti_can_edit_dashboard()` - Check edit access
- `ajasti_get_dashboard_url()` - Get dashboard path
...
```bash
# Get editor types
GET http://localhost/ajasti/api/editor/editor-types.php

# Get editor dashboard
GET http://localhost/ajasti/api/editor/dashboard.php
Header: Authorization: Bearer <session_token>

# Register as editor
POST http://localhost/ajasti/api/auth/register.php
{
  "editor_type": "section_editor",
  "first_name": "Test",
  ...
