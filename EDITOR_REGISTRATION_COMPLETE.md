...
- `jasti_ensure_editor_types()` - Create/update editor type tables on demand
- `jasti_editor_types()` - Retrieve all editor types
- `jasti_editor_type_by_id()` - Get type by ID
- `jasti_editor_type_by_name()` - Get type by name
- `jasti_user_editor_profile()` - Get user's editor profile with type details
- `jasti_create_editor_profile()` - Create editor profile during registration
- `jasti_can_access_dashboard()` - Check read permission for dashboard
- `jasti_can_edit_dashboard()` - Check edit permission for dashboard
- `jasti_get_dashboard_url()` - Get dashboard route based on editor type
...
- **Production**: Direct connection to your deployed JASTI API domain
- **CORS**: Configured for both localhost:5173 and production URLs

### Database Automatic Setup
The `jasti_ensure_editor_types()` function automatically:
...
- Production: https://your-jasti-domain.example/portal
