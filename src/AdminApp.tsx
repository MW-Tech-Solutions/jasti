...
function EicPanels({ section, workspace, onSaved }: { section: string; workspace: WorkspacePayload; onSaved: () => Promise<void> }) {
  ...
  const allManuscripts = asArray(workspace.admin?.manuscripts)
  ...
  if (section === "manage-all-articles") {
    return (
      <div className="grid gap-6">
        <Card className="min-w-0 border-white/70 bg-white/85 backdrop-blur">
          <CardHeader className="flex flex-wrap items-center justify-between gap-3">
            <CardHeading icon={<BookMarked className="h-5 w-5" />} title="Manage All Articles" description="View all manuscripts in the journal workspace and manage publication records." />
            <Input className="max-w-xs" placeholder="Search ref #, ID, title, status" value={manageArticlesSearch} onChange={(e) => setManageArticlesSearch(e.target.value)} />
          </CardHeader>
          <CardContent>
            <PublicationRecordsTable
              rows={filteredAllManuscripts}
              search={manageArticlesSearch}
              onSearchChange={(v) => setManageArticlesSearch(v)}
              deletingId={deletingId}
              onDelete={handleDeletePublication}
              onEdit={handleEditPublication}
              emptyMessage="No manuscripts are available."
            />
          </CardContent>
        </Card>
      </div>
...
function AdminManageAllArticlesPanel({ workspace, onSaved }: { workspace: WorkspacePayload; onSaved: () => Promise<void> }) {
    ...
    <>
      <div className="grid gap-6">
        <Card className="border-white/70 bg-white/85 backdrop-blur">
          <CardHeader className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <CardTitle>Manage All Articles</CardTitle>
              <CardDescription>View, search, edit, archive, and delete manuscripts or publications.</CardDescription>
            </div>
