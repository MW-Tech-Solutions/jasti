...
import { journalApi, resolveApiErrorMessage, type VerificationMeta } from "@/lib/journalApi"
...
export default function EditorRegistrationForm({ onBack, onSuccess, onAccountTypeChange, fixedAccountType }: EditorRegistrationFormProps) {
  ...
  const availableEditorTypes = React.useMemo(
    () => editorTypes.filter((type) => type.type_name !== "reviewer"),
    [editorTypes],
  )
  ...
  React.useEffect(() => {
    ...
    if (availableEditorTypes.some((type) => type.type_name === selectedType)) {
      return
    }

    setSelectedType("")
    setFormData((prev) => (prev.editor_type === selectedType ? { ...prev, editor_type: "" } : prev))
  }, [availableEditorTypes, selectedType])
  ...
  const handleRegisterClick = async () => {
    ...
    try {
      if (accountType === "author") {
        const { data } = await journalApi.post<RegistrationResponse>("/auth/create-account.php", {
          ...registrationPayload,
          confirm_password,
          role: "author",
        })
        if (data.verification?.email_sent === false) {
          toast.warning(data.message ?? "Registration successful, but the verification email could not be sent.")
        } else {
          toast.success(data.message ?? "Registration successful! Check your email to verify your account.")
        }
      ...
      const { data } = await journalApi.post<RegistrationResponse>("/auth/create-account.php", { ...registrationPayload, confirm_password, editor_type: selectedType, role: "editor" })
      if (data.verification?.email_sent === false) {
        toast.warning("Account created, but the verification email could not be sent. Please upload your CV, then contact support if resend still fails.")
      } else {
          ...
          <div>
            ...
            <div className="flex gap-4">
              ...
              <Button
                ...
                disabled={accountType === "editor" && (!selectedType || editorTypesLoading || editorTypesError !== "" || !availableEditorTypes.some((type) => type.type_name === selectedType))}
              >
                Save & Continue
              </Button>
            </div>
